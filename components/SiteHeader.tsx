...
const bottomNavItems: NavItem[] = [
  ...
  { href: "/today", label: "\u4eca\u65e5", icon: CalendarClock },
  { href: "/itinerary", label: "\u884c\u7a0b", icon: MapPinned },
  { href: "/expenses", label: "\u65c5\u8cbb", icon: ReceiptText },
  { href: "/bookings", label: "\u9810\u8a02", icon: ListChecks }
];
...
export function SiteHeader() {
  ...
  return (
    <>
      ...
      <nav
        className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-5 rounded-2xl bg-white/92 p-1 shadow-soft ring-1 ring-slate-200 backdrop-blur-xl dark:bg-lake-900/92 dark:ring-white/10 md:hidden"
        aria-label="\u624b\u6a5f\u5feb\u901f\u5c0e\u89bd"
      >
        {bottomNavItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "focus-ring flex flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[11px] font-black transition",
                active ? "bg-lake-900 text-white dark:bg-white dark:text-lake-900" : "text-slate-500 dark:text-slate-300"
              )}
            >
              <Icon className="h-4 w-4" aria-hidden="true" />
